...
static void scan(struct k_work *w){ARG_UNUSED(w);uint8_t rows=0;for(int c=0;c<6;c++){wr(IODIRA,0xFF);wr(OLATA,1U<<c);wr(IODIRA,(uint8_t)~(1U<<c));k_busy_wait(80);if(!rd(GPIOB,&rows)){for(int r=0;r<4;r++){bool x=(rows&(1U<<r))!=0;if(x==prev[r][c]){if(db[r][c]<2)db[r][c]++;}else{prev[r][c]=x;db[r][c]=0;}if(db[r][c]>=1&&x!=stable[r][c]){stable[r][c]=x;emit(keys[r][c],x);}}}}wr(IODIRA,0xFF);wr(OLATA,0);k_work_schedule(&work,K_MSEC(1));}
