...
static bool stable[4][6]; static uint8_t db[4][6];
...
static void scan(struct k_work *w){ARG_UNUSED(w);uint8_t rows=0;for(int c=0;c<6;c++){wr(OLATA,1U<<c);wr(IODIRA,(uint8_t)~(1U<<c));k_busy_wait(30);if(!rd(GPIOB,&rows)){for(int r=0;r<4;r++){bool x=(rows&(1U<<r))!=0;if(x){db[r][c]=0;if(!stable[r][c]){stable[r][c]=true;emit(keys[r][c],true);}}else if(stable[r][c]){if(++db[r][c]>=2){db[r][c]=0;stable[r][c]=false;emit(keys[r][c],false);}}else{db[r][c]=0;}}}wr(IODIRA,0xFF);}wr(OLATA,0);k_work_schedule(&work,K_MSEC(1));}
